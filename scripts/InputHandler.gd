...
func _release_detected(position: Vector2) -> void:
	...
	match touch_state:
		InputState.TOUCH_DOWN:
			if total_distance < tap_threshold and elapsed < tap_duration:
				# Quick tap
				_handle_tap()
			else:
				# Slow/no-movement release = position tap
		...
		InputState.DRAG:
			# Drag release = precision shot
			var drag_vector: Vector2 = position - touch_start_pos
